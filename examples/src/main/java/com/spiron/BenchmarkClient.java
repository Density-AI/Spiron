...
public class BenchmarkClient {
  ...
  static {
    ...
    for (int i = 0; i < NUM_CLUSTERS; i++) {
      CLUSTER_CENTERS.add(
        generateClusterCenter(VECTOR_DIMENSIONS, SHARED_RANDOM)
      );
    }
  }
  ...
  private static void checkNodesAvailable(int[] metricsPorts) {
    for (int i = 0; i < metricsPorts.length; i++) {
      try {
        URI uri = URI.create(
          "http://localhost:" + metricsPorts[i] + "/metrics"
        );
        java.net.HttpURLConnection conn = (java.net.HttpURLConnection) uri
          .toURL()
          .openConnection();
        ...
        if (responseCode == 200) {
          System.out.println(
            "  ✓ Node " + (i + 1) + " (port " + metricsPorts[i] + ") - ONLINE"
          );
        } else {
          System.out.println(
            "  ✗ Node " + (i + 1) + " - UNEXPECTED RESPONSE: " + responseCode
          );
        }
        conn.disconnect();
      } catch (Exception e) {
  ...
  private static void runWarmup(SpironClient client) throws Exception {
    ...
    for (int i = 0; i < WARMUP_REQUESTS; i++) {
      ...
      // Energy within validation bounds: [0.0, 0.75] -> use [0.3, 0.7] for safety
      double fuzzyEnergy = 0.3 + random.nextDouble() * 0.4;
      // Use cluster ID so vectors merge when they hit the same cluster
      EddyState eddy = new EddyState(
        "cluster-" + clusterId,
        vector,
        fuzzyEnergy,
        null
  ...
  private static BenchmarkResult runBenchmark(SpironClient client)
    throws Exception {
    ...
    for (int i = 0; i < NUM_REQUESTS; i++) {
      ...
      executor.submit(() -> {
        try {
          ...
          double[] vector = generateClusteredVector(VECTOR_DIMENSIONS, random);
          int clusterId = random.nextInt(NUM_CLUSTERS);
          // Energy within validation bounds: [0.0, 0.75] -> use [0.4, 0.7] for variation
          double fuzzyEnergy = 0.4 + random.nextDouble() * 0.3;
          ...
          if ((requestId + 1) % 10000 == 0) {
            System.out.println(
              "  Progress: " + (requestId + 1) + "/" + NUM_REQUESTS
            );
          }
        } catch (Exception e) {
    ...
    return new BenchmarkResult(
      ...
      failureCount.get(),
      totalDurationMs,
      (int) ((java.util.stream.IntStream.range(0, 10)
            .filter(i -> {
              try {
                java.net.URI uri = URI.create(
                  "http://localhost:" + (9091 + i) + "/metrics"
                );
   ...
   */
  private static double[] generateClusteredVector(
    int dimensions,
    Random random
  ) {
  ...
  private static void printResults(BenchmarkResult result) {
    ...
    System.out.println(
      ...
      String.format(
        "%.2f",
        (1000.0 * result.successCount) / result.totalDurationMs
      ) +
      " req/sec"
    );
    System.out.println();
    ...
    if (!result.latencies.isEmpty()) {
      ...
      System.out.println(
        "  Average:            " + String.format("%.2f", avg) + " ms"
      );
    ...
    System.out.println(
      "  Total Ingested:     " +
      String.format("%.2f", totalDataBytes / 1024.0 / 1024.0) +
      " MB"
    );
    System.out.println();
  }
