...
public class RpcClient {
  ...
  public void broadcast(EddyState state) {
    ...
    // SYNCHRONOUS: Execute broadcasts to all peers sequentially to ensure completion
    for (int i = 0; i < stubs.size(); i++) {
      ...
      try {
        ...
        executeWithRetry(() -> {
          if (rpcMetrics != null) {
            rpcMetrics.recordLatency(() -> stub.broadcast(msg));
  ...
  public void commit(EddyState state) {
    ...
    // SYNCHRONOUS: Execute commits to all peers sequentially to ensure completion
    for (int i = 0; i < stubs.size(); i++) {
      final var stub = stubs.get(i);
      final int peerIndex = i;
      ...
      try {
        ...
        executeWithRetry(() -> {
          if (rpcMetrics != null) {
            rpcMetrics.recordLatency(() -> stub.commit(env));
